...
class game:
    def __init__(self, N = 100, gameRounds = 2000):
        ...
        self.historyChoice = [] # 過去每一輪玩家所選擇的機器編號
        self.historyReward = [] # 過去每一輪玩家所獲得的糖果
    ...
    def run(self, agent1, agent2, log = False):
        while self.round < self.gameRounds:
            if self.round % 2 == 0: # 輪到 agent1行動
                choice = agent1.play() # agnet1 的play函數應該要return所選的機器編號
                reward = 1 if random.random() <= self.machine[choice] else 0
                self.agent1Reward += reward
                self.historyChoice.append(choice)
                ...
                choice = agent2.play() # agnet1 的play函數應該要return所選的機器編號
                reward = 1 if random.random() <= self.machine[choice] else 0
