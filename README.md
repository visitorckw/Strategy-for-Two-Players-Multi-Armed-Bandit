...
Both participants will work with the same set of <img src="http://latex.codecogs.com/gif.latex?\\N" /> vending machines (bandits). Each bandit provides a random reward based on a probability distribution specific to that machine. Every round each player selects ("pulls") a bandit, the likelihood of a reward decreases by 3%.
...
1. Pure Greedy base on Average Profit in history
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\Avg_i = \frac{w_i}{n_i}" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\w_i" />: Rewards we have gotten on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: Times we have played on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    
    Note that the strategy is straightforward and doesn't consider the effect of the decaying rate.
    
2. Epsilon-Delta Strategy

    The problem is an "exploitation" and "exploration" tradeoff dilemma. Epsilon-Delta is one of the simple and famous strategies. We choose a probability <img src="http://latex.codecogs.com/gif.latex?\\p" /> for exploration. Each time when we play, we use <img src="http://latex.codecogs.com/gif.latex?\\p" /> of chance randomly choose a machine to play (exploration), and another <img src="http://latex.codecogs.com/gif.latex?\\1-p" /> of the chance to play the machine that has highest average reward in history just like we mentioned in pure greedy strategy (exploitation).
    

    <img src="http://latex.codecogs.com/gif.latex?\\chosen \ machine = \begin{cases} random, & {chance \ of \ p} \\ argmax \ Avg_i, & {chance \ of \ 1 - p} \end{cases}" />

3. UCB Strategy

    UCB formula is one of the famous mathematical methods for balancing the "exploitation" and "exploration" strategy. Each time when we play, we choose a machine <img src="http://latex.codecogs.com/gif.latex?\\i" />  which makes the following formula has the highest value:
    
    <img src="http://latex.codecogs.com/gif.latex?\\UCB = \frac{w_i}{n_i} + c * \sqrt{\frac{ln{N}}{n_i}}" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\w_i" />: Rewards we have gotten on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: Times we have played on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    
    <img src="http://latex.codecogs.com/gif.latex?\\c" />: Exploration parameter, equals to <img src="http://latex.codecogs.com/gif.latex?\sqrt{2}" /> in theory.
    <img src="http://latex.codecogs.com/gif.latex?\\N" />: Times we have played on all machines in history.
4. Advance Greedy Method base on Maximum Likelihood Estimation

    We would like to maximize our expected profit each time we play based on the observation of historical results. To know the expected profit of each machine, we have to consider the effect of the decaying rate. For a specific machine <img src="http://latex.codecogs.com/gif.latex?\\i" />, we have history information <img src="http://latex.codecogs.com/gif.latex?\\I" />, representing in a series <img src="http://latex.codecogs.com/gif.latex?\\(player_j, w_j)" />. If we have a given probability <img src="http://latex.codecogs.com/gif.latex?\\p" /> for machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> beginning expected profit, we could calculate <img src="http://latex.codecogs.com/gif.latex?\\P(I|p)" /> easily:
    

    <img src="http://latex.codecogs.com/gif.latex?\\P(I|p) = \prod_{j=0}{\begin{cases} (0.97)^j * p, & {  player_j = me \land w_j = 1} \\  (1-p*(0.97)^j), & {  player_j = me \land w_j = 0} \\ 1, & {  otherwise} \end{cases}}" />
    
    However, what we want to know is the value of <img src="http://latex.codecogs.com/gif.latex?\\p" />. So we could try to find the value <img src="http://latex.codecogs.com/gif.latex?\\p = p*" /> which makes <img src="http://latex.codecogs.com/gif.latex?\\P(p|I)" /> have the highest value. We use the likelihood function <img src="http://latex.codecogs.com/gif.latex?\\L(p|I) = P(I|p)" /> to find the maximum likelihood estimation of <img src="http://latex.codecogs.com/gif.latex?\\p" />.
    Each time we play, we find a probability <img src="http://latex.codecogs.com/gif.latex?\\p_i" /> for machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> which has maximum likelihood. In practice, we use interior-point method to find the optimal value <img src="http://latex.codecogs.com/gif.latex?\\p*" />. Then we choose the machine which has the highest expected value. The expected value for machine <img src="http://latex.codecogs.com/gif.latex?\\i" />:
    

    <img src="http://latex.codecogs.com/gif.latex?\\EV_i = p_i * (0.97) ^ {n_i}" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\p_i" />: maximum likelihood estimation of <img src="http://latex.codecogs.com/gif.latex?\\p" /> on machine <img src="http://latex.codecogs.com/gif.latex?\\i" />.
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: times we and out opponent have played on mahine <img src="http://latex.codecogs.com/gif.latex?\\i" />.
...
5. Advance UCB Method by Combining Advance Greedy and Pure UCB
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\newUCB = EV_i + c \sqrt{\frac{\ln{N}}{n_i}}" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\EV_i" />: Ecpected value of machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> by calculating maximum likelihood.
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: Times we play on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    <img src="http://latex.codecogs.com/gif.latex?\\c" />: Exploration parameter, equals to <img src="http://latex.codecogs.com/gif.latex?\sqrt{2}" /> in theory.
    <img src="http://latex.codecogs.com/gif.latex?\\N" />: Times we play on all machines in history.
...
7. Combining Advance UCB with a ML/DL model:
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\newUCB = y_i + c \sqrt{\frac{\ln{N}}{n_i}}" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\EV_i" />: Ecpected value of machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> predicted by the ML model.
    
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: Times we have played on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    <img src="http://latex.codecogs.com/gif.latex?\\c" />: Exploration parameter, equals to <img src="http://latex.codecogs.com/gif.latex?\sqrt{2}" /> in theory.
    <img src="http://latex.codecogs.com/gif.latex?\\N" />: Times we have played on all machines in history.
    <img src="http://latex.codecogs.com/gif.latex?\\y_i" />: Output of the ML/DL model on machine <img src="http://latex.codecogs.com/gif.latex?\\i" />.
...
8. Thompson Sample and Bayesian Distribution
    ...
    * For each round <img src="http://latex.codecogs.com/gif.latex?\\t = 1,2,..." /> do
        * calculate <img src="http://latex.codecogs.com/gif.latex?\\N^{1}_{i}$ and $N^{0}_{i}" />
            * <img src="http://latex.codecogs.com/gif.latex?\\N^{1}_{i}" /> = the amount of candy get from machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> 
            * <img src="http://latex.codecogs.com/gif.latex?\\N^{0}_{i}" /> = the amount round that didn't get candy from machine  <img src="http://latex.codecogs.com/gif.latex?\\i" /> 
        * <img src="http://latex.codecogs.com/gif.latex?\\theta_{i}(t)$ $\approx$ $\beta(N^{1}_{i},N^{0}_{i})" />
        * choose the machine <img src="http://latex.codecogs.com/gif.latex?\\k" /> biggest <img src="http://latex.codecogs.com/gif.latex?\theta_{k}(t)" />
...
7. Gradient Bandit Algorithm
    ...
    For each possible action, we have a preference value <img src="http://latex.codecogs.com/gif.latex?\\H" />. The probability of each action will be chosen to obey the softmax distribution. The formula is as follows:


    <img src="http://latex.codecogs.com/gif.latex?\\Pr(a) = \pi(a) = \frac{e^{H(a)}}{\sum{e^{H(b)}}}, b \in Action" />
    
    <img src="http://latex.codecogs.com/gif.latex?\pi" /> denotes our playing strategy. The preference value <img src="http://latex.codecogs.com/gif.latex?\\H" /> will initialize to <img src="http://latex.codecogs.com/gif.latex?\0" /> for all actions. Then we apply SGD algorithm to our strategy for updating preference value. After we make an action <img src="http://latex.codecogs.com/gif.latex?\\A_t" />, we could update by using the formula below:


    <img src="http://latex.codecogs.com/gif.latex?\\H(A_t)' = H(A_t) + \alpha(R - \bar R)(1 - \pi(A_t))" />

    <img src="http://latex.codecogs.com/gif.latex?\\H(a)' = H(a) - \alpha(R - \bar R)\pi(a), \forall a \neq A_t" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\H'" />: New preference value.
    <img src="http://latex.codecogs.com/gif.latex?\\H" />: Old preference value.
    <img src="http://latex.codecogs.com/gif.latex?\\\alpha" />: Learing rate.
    <img src="http://latex.codecogs.com/gif.latex?\\R" />: Last reward
    <img src="http://latex.codecogs.com/gif.latex?\\\bar R" />: Average reward
...
9. Exponential smoothing predict strategy
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\EV' = (1 - \alpha)EV + \alpha R" />
    
    <img src="http://latex.codecogs.com/gif.latex?\\EV'" />: New prediction
    <img src="http://latex.codecogs.com/gif.latex?\\EV" />: Old prediction
    <img src="http://latex.codecogs.com/gif.latex?\\\alpha" />: A hyperparameter controls the influence of new results.
    <img src="http://latex.codecogs.com/gif.latex?\\R" />: Accurate result.
...
13. Adaptive UCB with exponential smoothing exploratin paramter
    ...
    We try to find a way to change the value of the exploration parameter in UCB formula dynamically. Intuitively speaking, when we observe the agent exploiting, we will want to encourage the agent to try more exploration and vice versa. So we compare the decisions made with UCB and greed. If the decisions are the same, we will increase the value of <img src="http://latex.codecogs.com/gif.latex?\\c" />. Otherwise, we will decrease <img src="http://latex.codecogs.com/gif.latex?\\c" />. The updated formula for <img src="http://latex.codecogs.com/gif.latex?\\c" />:
    

    <img src="http://latex.codecogs.com/gif.latex?\\c' = c * 1.1, if A = B\\\ c' = c * 0.9, if A \neq B" />,
    
    
    
    <img src="http://latex.codecogs.com/gif.latex?\\c'" />: New value of c
    <img src="http://latex.codecogs.com/gif.latex?\\c" />: Old value of c
    <img src="http://latex.codecogs.com/gif.latex?\\A" />: UCB dicision
    <img src="http://latex.codecogs.com/gif.latex?\\B" />: greedy dicision 
...
14. Combining ML method with adaptive UCB strategy
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\EV_i" />: Ecpected value of machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> predicted by the ML model.
    <img src="http://latex.codecogs.com/gif.latex?\\n_i" />: Times we have played on machine <img src="http://latex.codecogs.com/gif.latex?\\i" /> in history.
    <img src="http://latex.codecogs.com/gif.latex?\\c" />: Exploration parameter, changes dynamically using exponential smooth strategy.
    <img src="http://latex.codecogs.com/gif.latex?\\N" />: Times we have played on all machines in history.
    <img src="http://latex.codecogs.com/gif.latex?\\y_i" />: Output of the ML/DL model on machine <img src="http://latex.codecogs.com/gif.latex?\\i" />.
...
In deep Q learning, we use the features mentioned above in machine learning as the state and the action is to choose which machine.
    ...
    <img src="http://latex.codecogs.com/gif.latex?\\Q(s,a)=r+\gamma\max_{a'}Q(s',a')\\\L=(r+\gamma\max_{a'}Q(s',a')-Q(s,a))^2" />
...
1. ELO rating system analysis
    ...
    The Elo rating system is a method for calculating the relative skill levels of players in zero-sum games such as chess. It is named after its creator Arpad Elo, a Hungarian-American physics professor. We compete with all the agents with each other. If agent A has a rating of <img src="http://latex.codecogs.com/gif.latex?\\R_A" /> and agent B rating of <img src="http://latex.codecogs.com/gif.latex?\\R_B" />, the exact formula (using the logistic curve with base 10) for the expected score of player A is
    
    
    <img src="http://latex.codecogs.com/gif.latex?\\E_A = \frac{1}{1+10^{(R_B-R_A)/400}}" />
    
    Similarly the expected score for agent B is
    
    
    <img src="http://latex.codecogs.com/gif.latex?\\E_B = \frac{1}{1+10^{(R_A-R_B)/400}}" />
    
    Suppose player A (again with rating <img src="http://latex.codecogs.com/gif.latex?\\R_A" />) was expected to score <img src="http://latex.codecogs.com/gif.latex?\\E_A" /> points but actually scored <img src="http://latex.codecogs.com/gif.latex?\\S_A" /> points <img src="http://latex.codecogs.com/gif.latex?\\win = 1, tie = 0.5, lose = 0)" />. The formula for updating that player's rating is
    
    
    <img src="http://latex.codecogs.com/gif.latex?\\R^{'}_{A} = R_A + K(S_A - E_A)\\ R^{'}_{B} = R_B + K(S_B - E_B)" />
...
4. In the UCB formula, we have a hyperparameter - "c" which is theoretically equal to <img src="http://latex.codecogs.com/gif.latex?\\\sqrt2" />. However, we found that The smaller the "c", the better the performance of the model. So we thought "exploitation" is much more important than "exploration" in this game.
